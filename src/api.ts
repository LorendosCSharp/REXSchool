...
export class API {
  ...
  private async getCSRFToken(url: string): Promise<string> {
    ...
    const tokens = root.querySelectorAll(
  ...
  async sendCode() {
    ...
    formData.append("__RequestVerificationToken", csrfToken);
    const response = await fetch(
      this.endpoint + "/Account/SendCode?ReturnUrl=%2F",
      {
        method: "POST",
        body: formData,
        headers: {
  ...
  async login() {
    if (this.username != undefined && this.password != undefined) {
      ...
      const loginResponse = await fetch(
        this.endpoint + "/Account/Login?ReturnUrl=%2F",
        {
          method: "POST",
      ...
      if (
        ...
        loginResponse.headers.has("set-cookie")
      ) {
        this.cookie.parseCookiesFromSet(

...
export class API {
  ...
  private async getCSRFToken(url: string): Promise<string> {
    ...
    if (response.headers.has('set-cookie'))
      this.cookie.parseCookiesFromSet([response.headers.get('set-cookie')!])
    console.log('[DEBUG] Cookies CSRF', this.cookie)
    console.log('[DEBUG] Cookies CSRF Header', response)
  ...
  async login() {
    if (this.username != undefined && this.password != undefined) {
      ...
      if (loginResponse.status === 302 && loginResponse.headers.has('set-cookie')) {
        this.cookie.parseCookiesFromSet([loginResponse.headers.get('set-cookie')!])
        console.log('[DEBUG] Cookies', this.cookie)
        this.logged = true
